...
3) An item is covered by an option if that option contains the item. Given a set of options Os and an item I to be covered write a predicate iscovered (I , Os , O). 
This predicate fails if I can not be covered by the options in Os and succeeds with O being a covering option. 
